...
# === Category Map (You can add more later) ===
category_map = {
    "Paper Products": [
...
def scrape_subcategory(driver, category, slug):
    ...
    while True:
        ...
        for link in product_links:
            ...
            # 🧾 Clean Product Details from review-wrapper
            details = {}
            try:
                ...
                for tag in strong_tags:
                    ...
                    # Regex to extract value after <strong>label:</strong>: value
                    match = re.search(rf"<strong[^>]*>{re.escape(label)}:?<\/strong>\s*:?([\s\S]+?)(<|$)", html)
                    value = match.group(1).strip() if match else ""

                    # Clean up unwanted trailing tags or whitespace
